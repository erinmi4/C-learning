...
string randomShuffle(string input) {
    if (input.empty()) {
        ...
        //随机取一张牌，然后将其放到顶部，也就是[0]的位置
        int randomNumber = randomInteger(0,input.length() - 1);
        return input[randomNumber] + randomShuffle(input.substr(0,randomNumber) + input.substr(randomNumber + 1))  ;
    }
}
